...
function fBuscarListaProventosBDR( urlPadrao ){
	try{

		 promise = new Promise( (resolve, reject) => {

			$("#DivListaCalendarioBDR").html("");

			var CodAtivo  = $("#txtFiltroCalendBdr").val();
			if( !CodAtivo || CodAtivo == "" ) 	 return;

			$("#txtFiltroCalendBdr").prop('disabled', true );
			$("#DivListaCalendarioBDR").html(
			...
			$.ajax({
				...
				data: { TipoInvest: 'BDR', CodAtivo: CodAtivo },
				success: function(result) {
					...
					if (resultado == "NSESSAO") {
						$("#DivListaCalendarioBDR").html("");
						$("#txtFiltroCalendBdr").prop('disabled', false );
						...
						if ( lista && lista.length > 0 ){
							$.each(lista, function (index, value) {
								...
								if ( EmprProvDtPagto == "" ) EmprProvDtPagto = 'SEM PREVISÃO';
								 ...
								 if ( EmprProvTipo == 'J' ) EmprProvTipoDescr = 'JRS CAP PRÓPRIO';
						...
						$("#DivListaCalendarioBDR").html(Conteudo);
						$("#txtFiltroCalendBdr").prop('disabled', false );
...
function fCarregarListaProventoBDR( urlPadrao ){
	try {

		 promise = new Promise( (resolve, reject) => {

			$('#calendarBDR').fullCalendar({
				 ...
				 eventSources: [
				   ...
				   // INICIO - LISTA DE DATA EX
				   {
					  ...
					  events: function(start, end, timezone, callback) {
						...
						$.ajax({
							...
							async   : true,
							type    : "POST",
							data    : { TipoInvest: 'BDR', TpLista : 'DTEX', DataIni: start.format('YYYYMMDD'), DataFim: end.format('YYYYMMDD'), },
							error: function() {
								fCriarAlertaPrinc(TP_ALERTA_ERRO, MSG_ALERTA_ERRO);
								return false;
							},
				   ...
				   // INICIO - LISTA DE DATA PAGTO
				   {
						...
						$.ajax({
							...
							async   : true,
							type    : "POST",
							data    : { TipoInvest: 'BDR', TpLista : 'DTPAGTO', DataIni: start.format('YYYYMMDD'), DataFim: end.format('YYYYMMDD'), },
							error: function() {
								fCriarAlertaPrinc(TP_ALERTA_ERRO, MSG_ALERTA_ERRO);
								return false;
							},
				   ...
				   // INICIO - LISTA DE OPERACOES DE COMPRA
				   {
					...
					$.ajax({
						...
						async   : true,
						type    : "POST",
						data    : { TipoInvest: 'BDR', TpLista: 'COMPRA', DataIni: start.format('YYYYMMDD'), DataFim: end.format('YYYYMMDD'), },
						error: function() {
							fCriarAlertaPrinc(TP_ALERTA_ERRO, MSG_ALERTA_ERRO);
							return false;
						},
				...
				// INICIO - LISTA DE OPERACOES DE VENDA
				 {
					...
					$.ajax({
						...
						async   : true,
						type    : "POST",
						data    : { TipoInvest: 'BDR', TpLista: 'VENDA', DataIni: start.format('YYYYMMDD'), DataFim: end.format('YYYYMMDD'), },
						error: function() {
							fCriarAlertaPrinc(TP_ALERTA_ERRO, MSG_ALERTA_ERRO);
							return false;
						},
				...
				// INICIO - LISTA DE GRUPAMENTO E DESDOBRAMENTO
				 {
					...
					$.ajax({
						...
						async   : true,
						type    : "POST",
						data    : { TipoInvest: 'BDR', TpLista:'OUTROS', DataIni: start.format('YYYYMMDD'), DataFim:  end.format('YYYYMMDD'), },
						error: function() {
							fCriarAlertaPrinc(TP_ALERTA_ERRO, MSG_ALERTA_ERRO);
							return false;
						},
						...
						content   : function() {
							...
							if ( event.provTipo ){
								if ( event.provTipo == "D" || event.provTipo == "J" || event.provTipo == "R" ){
									...
									if ( event.provTipo == 'J') ProvTipo = 'JRS CAP PRÓPRIO';
