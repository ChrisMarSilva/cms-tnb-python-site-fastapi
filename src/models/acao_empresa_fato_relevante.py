...
class ACAOEmpresaFatoRelevanteModel(_database.session.Base):

    __tablename__ = "TBEMPRESA_FATORELEVANTE"
    ...
    nm_empresa = _sql.Column('NMEMPRESA', _sql.String(250), nullable=True, index=True)
    ...
    def data_env_format(self) -> str:
        return converter_datetime_str(data=converter_str_to_datetime(data=self.data_env, fmt='%Y%m%d%H%M%S'), fmt='%d/%m/%Y')
    ...
    def data_env_format_xml(self) -> str:
        return converter_datetime_str(data=converter_str_to_datetime(data=self.data_env, fmt='%Y%m%d%H%M%S'), fmt='%Y-%m-%d')
    ...
    def __repr__(self):
        return '<EmpresaFatoRelevante {str(self.id)} - {self.nome} - {self.cnpj}>'
