...
class ETFIndiceCotacaoModel(_database.session.Base):

    __tablename__ = "TBETF_INDICE_COTACAO"

    data = _sql.Column('DATA', _sql.String(8), nullable=False, index=True)
    id_indice = _sql.Column('IDINDICE', _sql.Integer, _sql.ForeignKey('TBETF_INDICE.ID'), primary_key=True, nullable=False, index=True)
