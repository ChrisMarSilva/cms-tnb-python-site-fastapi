...
class FiiFundoImobCotacaoModel(_database.session.Base):

    __tablename__ = "TBFII_FUNDOIMOB_COTACAO"

    data = _sql.Column('DATA', _sql.String(8), nullable=False, index=True)
    id_fundo = _sql.Column('IDFUNDO', _sql.Integer, _sql.ForeignKey('TBFII_FUNDOIMOB.ID'), primary_key=True, nullable=False, index=True)
