...
class ACAOEmpresaAtivoCotacaoModel(_database.session.Base):

    __tablename__ = "TBEMPRESA_ATIVOCOTACAO"

    data = _sql.Column('DATA', _sql.String(8), nullable=False, index=True)
    id_ativo = _sql.Column('IDATIVO', _sql.Integer, _sql.ForeignKey('TBEMPRESA_ATIVO.ID'), primary_key=True, nullable=False, index=True)
