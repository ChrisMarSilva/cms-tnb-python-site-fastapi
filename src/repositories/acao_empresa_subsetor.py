...
from src.models.acao_empresa_subsetor import ACAOEmpresaSubSetorModel
...
class ACAOEmpresaSubSetorRepository:
    ...
    async def get_all(cls, db: _orm.Session):
        try:
            return db.query(ACAOEmpresaSubSetorModel).filter_by(situacao='A').order_by(ACAOEmpresaSubSetorModel.descricao).all()
    ...
    async def get_by_id(cls, db: _orm.Session, id: int):
        try:
            return db.query(ACAOEmpresaSubSetorModel).filter_by(id=id, situacao='A').first()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    async def get_by_descricao(cls, db: _orm.Session, descricao: str):
        try:
            return db.query(ACAOEmpresaSubSetorModel).filter_by(descricao=descricao, situacao='A').first()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
