...
from src.models.usuario_acao_empresa_aluguel import UsuarioACAOEmpresaAluguelModel
...
class UsuarioACAOEmpresaAluguelRepository:
    ...
    async def get_all(cls, db: _orm.Session):
        try:
            return db.query(UsuarioACAOEmpresaAluguelModel).all()
    ...
    async def get_all_by_usuario(cls, db: _orm.Session, id_usuario: int):
        try:
            return db.query(UsuarioACAOEmpresaAluguelModel).filter_by(id_usuario=id_usuario).order_by(UsuarioACAOEmpresaAluguelModel.data, cls.id_ativo).all()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    async def get_by_id(cls, db: _orm.Session, id: int):
        try:
            return db.query(UsuarioACAOEmpresaAluguelModel).filter_by(id=id).first()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    async def get_by_usuario(cls, db: _orm.Session, id_usuario: int, id: int):
        try:
            return db.query(UsuarioACAOEmpresaAluguelModel).filter_by(id_usuario=id_usuario, id=id).first()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    #     .filter(MODEL.situacao == 'A', UsuarioEmpresaAluguel.id_usuario == id_usuario)\
    #     .order_by(MODEL.codigo)\
    #     .all()
    ...
    # filters = []
    # filters.append(MODEL.id_usuario == id_usuario)
    # if cod_ativo: filters.append(EmpresaAtivo.codigo == cod_ativo)
    # if dt_ini: filters.append(MODEL.data >= dt_ini)
    # if dt_fim: filters.append(MODEL.data <= dt_fim)
    # return cls.query\
    #     .join(EmpresaAtivo, EmpresaAtivo.id == cls.id_ativo) \
    #     .filter(*filters) \
    #     .add_columns(EmpresaAtivo.codigo) \
    #     .order_by(MODEL.data, cls.id_ativo)\
    #     .all()
    ...
    @classmethod
    async def salvar(cls, db: _orm.Session, row: UsuarioACAOEmpresaAluguelModel, commit: bool = True):
        try:
            db.add(row)
            if commit: db.commit()
        except Exception as e:
    ...
    @classmethod
    async def excluir(cls, db: _orm.Session, row: UsuarioACAOEmpresaAluguelModel, commit: bool = True):
        try:
            db.delete(row)
            if commit: db.commit()
        except Exception as e:
