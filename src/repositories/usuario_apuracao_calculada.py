...
# class UsuarioApuracaoCalculada(db.Model):
...
#     vlr_venda = db.Column('VLR_VENDA', db.Float(17, 2), nullable=True)
#     vlr_lucro_apurado = db.Column('VLR_LUCRO_APURADO', db.Float(17, 2), nullable=True)
#     vlr_prejuizo_compensar = db.Column('VLR_PREJUIZO_COMPENSAR', db.Float(17, 2), nullable=True)
#     vlr_base_calculo = db.Column('VLR_BASE_CALCULO', db.Float(17, 2), nullable=True)
#     vlr_ir_devido = db.Column('VLR_IR_DEVIDO', db.Float(17, 2), nullable=True)
#     vlr_ir_pago = db.Column('VLR_IR_PAGO', db.Float(17, 2), nullable=True)
#     vlr_ir_pagar = db.Column('VLR_IR_PAGAR', db.Float(17, 2), nullable=True)
...
#     @classmethod
#     def gerar_todos(cls, id_usuario: int) -> None:
#         try:
...
#                 tasks = [
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='C')),  # C - ACAO Comum
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='D')),  # D - ACAO DayTrade
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='F')),  # F - FII
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='E')),  # E - ETF Comum
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='G')),  # G - ETF DayTrade
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='I')),  # I - BDR Comum
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='J')),  # J - BDR Day Trade
#                     asyncio.Task(cls.gerar_async(id_usuario=id_usuario, tp_apuracao='K')),  # K - Cripto
#                 ]
...
#     @classmethod
#     @asyncio.coroutine
#     async def gerar_async(cls, id_usuario: int, tp_apuracao: str, calc_vlr_superior: str = None) -> None:
#         cls.gerar(id_usuario=id_usuario, tp_apuracao=tp_apuracao, calc_vlr_superior=calc_vlr_superior)

#     @classmethod
#     def gerar(cls, id_usuario: int, tp_apuracao: str, calc_vlr_superior: str = None) -> None:
#         try:
...
#     @classmethod
#     def get_all(cls, id_usuario: int = None, categoria: str = None, ano_mes_ini: str = None, ano_mes_fim: str = None):
#         try:
#             filters = []
#             if id_usuario: filters.append(cls.id_usuario == id_usuario)
#             if categoria: filters.append(cls.categoria == categoria)
#             if ano_mes_ini: filters.append(cls.ano_mes >= ano_mes_ini)
#             if ano_mes_fim: filters.append(cls.ano_mes <= ano_mes_fim)
#             return cls.query.filter(*filters).order_by(cls.ano_mes).all()
#         except Exception as e:
#             LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
#             raise

#     @classmethod
#     def get_total(cls, id_usuario: int = None, categoria: str = None, ano_mes_ini: str = None, ano_mes_fim: str = None):
#         try:
#             filters = []
#             if id_usuario: filters.append(cls.id_usuario == id_usuario)
#             if categoria: filters.append(cls.categoria == categoria)
...
#     @classmethod
#     def excluir_tudo_por_categoria(cls, id_usuario: int, categoria: str, commit: bool = True):
#         try:
