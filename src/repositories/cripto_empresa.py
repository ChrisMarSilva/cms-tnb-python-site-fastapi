...
class CriptoEmpresaRepository:
    ...
    async def get_all(cls, db: _orm.Session):
        try:
            return db.query(CriptoEmpresaModel).order_by(CriptoEmpresaModel.nome).all()
    ...
    async def get_by_id(cls, db: _orm.Session, id: int):
        try:
            return db.query(CriptoEmpresaModel).filter_by(id=id).first()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    async def get_by_codigo(cls, db: _orm.Session, codigo: str):
        try:
            return db.query(CriptoEmpresaModel).filter_by(codigo=codigo).first()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    async def get_lista_nomes(cls, db: _orm.Session):
        try:
            return db.query(CriptoEmpresaModel).filter(CriptoEmpresaModel.situacao.in_(['A', 'E'])).order_by(CriptoEmpresaModel.nome).all()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    async def get_lista_codigos(cls, db: _orm.Session):
        try:
            return db.query(CriptoEmpresaModel).filter(CriptoEmpresaModel.situacao.in_(['A', 'E'])).order_by(CriptoEmpresaModel.codigo).all()
        except Exception as e:
            #  LogErro.registrar(texto=str(e), arqv=str(os.path.basename(__file__).replace('.py', '') + '.' + __class__.__name__), linha=int(sys.exc_info()[-1].tb_lineno))
            raise
    ...
    @classmethod
    async def salvar(cls, db: _orm.Session, row: CriptoEmpresaModel, commit: bool = True):
        try:
            db.add(row)
            if commit: db.commit()
        except Exception as e:
    ...
    @classmethod
    async def excluir(cls, db: _orm.Session, row: CriptoEmpresaModel, commit: bool = True):
        try:
            db.delete(row)
            if commit: db.commit()
        except Exception as e:
