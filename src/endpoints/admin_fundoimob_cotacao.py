...
@router.post(path='/salvar', status_code=_fastapi.status.HTTP_200_OK)
...
@router.post(path='/AtualizaCotacao', status_code=_fastapi.status.HTTP_200_OK)
async def atualiza_cotacao(db: _orm.Session = _fastapi.Depends(_database.base.get_db), current_user=_fastapi.Depends(manager)):
    with _tracer.tracer.start_as_current_span(f"{str(os.path.basename(__file__).replace('.py', ''))}.{inspect.stack()[0][3]}") as span:
        # dados = await xxxxService.xxxxxxx(db=db)
        # return dados
        pass
